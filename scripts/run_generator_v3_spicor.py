...
def checkpoint_model(checkpoint, vocab, device):
    ...
    model = SwaraSpeechGeneratorV3(config, vocab, (SPEAKER,))
...
def main():
    ...
    ap.add_argument("--run", type=Path, default=Path("runs/generator_v3_spicor_30min_v0"))
    ...
    if args.evaluate_only:
        ...
        model = SwaraSpeechGeneratorV3(config, vocab, (SPEAKER,)); model.module.to(device)
    ...
    while step < max_steps:
        ...
        if step in eval_steps or step == max_steps:
            tr = metrics(model, train_items[: min(64, len(train_items))]); va = metrics(model, val_items); entry = {"step": step, "train": tr, "validation": va}; history.append(entry); print(json.dumps(entry), flush=True)
    ...
    summary = {"seed": SEED, "device": str(device), "parameter_count": model.parameter_count, "steps": step, "wall_clock_seconds": time.time() - start, "history": history, "best": best_metrics}
