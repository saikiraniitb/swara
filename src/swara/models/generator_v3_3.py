"""Swara Generator v3.3 with independent residual output heads.
...
class SwaraSpeechGeneratorV33:
    ...
    def __init__(self, config: GeneratorV3Config, vocabulary: LinguisticVocabulary, speaker_ids: tuple[str, ...]):
        ...
        self.module = _V33Module(config, nn, kinds, languages, kind_count, language_count)
    ...
    def __new__(cls, config, nn, kinds, languages, kind_count, language_count):
        ...
        class Module(nn.Module):
            def __init__(self):
                ...
                self.residual_cell = nn.GRUCell(d, d)
                self.residual_heads = nn.ModuleList([nn.Linear(d, config.audio_spec.vocabulary_size) for _ in range(15)])
            ...
            def residual_logits(self, hidden, primary, targets=None):
                ...
                for i in range(15):
                    cb=self.residual_codebook.weight[i].view(1,1,-1); state=self.residual_cell((self.residual_prev(previous)+cb).reshape(-1,d), state.reshape(-1,d)).view(b,n,d); out.append(self.residual_heads[i](state+cb)); previous=targets[:,:,i] if targets is not None else out[-1].argmax(-1)
